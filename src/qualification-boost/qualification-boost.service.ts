...
@Injectable()
export class QualificationBoostService extends TypeOrmCrudService<QualificationBoost> {
  constructor(
  ...
  public async updateQualificationBoost(
    ...
    if (!qualificationBoost)
      throw new NotFoundException(errorMessage.qualificationBoostNotFound);
  ...
  private contains(arr: string[]) {
    const indexArray = arr.map((currentValue) =>
      [CodeTypeE.BEZ_LAKTOZE, CodeTypeE.D_VITAMIN, CodeTypeE.KOZJE].indexOf(
        +currentValue,
      ),
    );
