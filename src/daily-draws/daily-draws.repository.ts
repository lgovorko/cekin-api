  ...
  public async getDailyDrawStats(dailyDrawId: number): Promise<any> {
    return this.query(`
            SELECT id, draw_date as drawDate,
              COALESCE((SELECT COUNT(*) FROM user_draw_qualifications WHERE daily_draw_id = ${dailyDrawId} AND type in (${CodeTypeE.BEZ_LAKTOZE},${CodeTypeE.D_VITAMIN},${CodeTypeE.KOZJE}) GROUP BY daily_draw_id), 0)::int as "codeEntry",
