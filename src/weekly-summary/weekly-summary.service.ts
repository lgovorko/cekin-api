...
export class WeeklySummaryService extends TypeOrmCrudService<WeeklySummary> {
  ...
  public async generateDailySummaryXlsx(
    ...
    const dataForExcel = usersRegistrationCountByDate.map(({ date, total }) => {
  ...
  private parseUsedCodes(
    ...
    const usedCodes = usedCodesGroupedByDateAndType.filter(
      (currentUsedCode) =>
        moment(currentUsedCode.date).format('YYYY-MM-DD') === currentDate,
    );

    return usedCodes.reduce(
      (prev, curr) => {
        prev.kozje = curr.type === CodeTypeE.KOZJE ? curr.total : prev.kozje;
        prev.kalcij =
          curr.type === CodeTypeE.D_VITAMIN ? curr.total : prev.kalcij;
        prev.bezLaktoze =
          curr.type === CodeTypeE.BEZ_LAKTOZE ? curr.total : prev.bezLaktoze;
        return prev;
      },
      {
        kozje: 0,
  ...
  private parseUserCodes(
    ...
    const userCodes = userCodeGroupedByDateAndStatus.filter(
      (currentUserCode) =>
        moment(currentUserCode.date).format('YYYY-MM-DD') === currentDate,
    );

    return userCodes.reduce(
      (prev, curr) => {
        prev.validCodes =
          curr.status === UserCodeStatusE.VALID ? curr.total : prev.validCodes;
        prev.invalidCodes =
