...
@Injectable()
export class UserDrawQualificationsService extends TypeOrmCrudService<UserDrawQualification> {
  constructor(
  ...
  public async share(
    ...
    const { qualificationsCount, extraSpent, isShared } = userDrawQualification;
  ...
  private async codeEntryQualificationValue(
    ...
    const qualificationBoost = await getRepository(QualificationBoost).findOne({
      where: {
  ...
  private checkCodeType(
    ...
    const ct = codeTypes.split(',').map((currentCodeType) => +currentCodeType);
  ...
  private async codeNotFound(userId: number, userCode: string): Promise<void> {
    await getRepository(UserCode).save({
      userId,
      userEntry: userCode,
    ...
    await this.redisInvalidCodeLockService.checkInvalidCodeEntryLimit(userId, {
